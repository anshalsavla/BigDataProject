...
def getDataType(x):
    ...
    return str(str(x)+', '+str(typ).replace('<class', '').strip('>').strip("'")+', '+'Closed Date, '+label)



if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: DueDate.py <file>")
        exit(-1)
    sc = SparkContext()
    csvfile = sc.textFile(sys.argv[1],1)
    dates = csvfile.mapPartitions(lambda x: csv.reader(x)).map(lambda x: x[20])
    base_type=dates.map(lambda x: getDataType(x))
    base_type.saveAsTextFile("DueDate.txt")
    sc.stop()
